...
export type FieldConfigurator<U extends any[], C> = <T>(
...
export function FieldConfigurator(getConfig?): FieldConfigurator<any, any> {
  return (configure: (config) => (field: Field) => void) => {
    const fieldToConfig = new WeakMap<Field, any>();

    Configurator.load = (field) => fieldToConfig.get(field);

    return Configurator;

    function Configurator(this: any, ...args) {
      const config = getConfig ? getConfig.apply(this, args) : args[0];
