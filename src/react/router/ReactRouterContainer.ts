...
export type ReactRouterContainerProps<T extends AnyReactRouterOld> = {
  history: History;
  router: T;
  children?: ReactNode;
...
export function ReactRouterContainer<T extends AnyReactRouterOld>(
  ...
  // @ts-ignore
  const initRoute: Route<AnyReactRouterOld> = useMemo(() => {
    return createRoute({
      parent: undefined,
      instance: { history },
      name: undefined,
