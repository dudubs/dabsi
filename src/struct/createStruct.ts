...
export declare type StructProps<T> = {
    [P in keyof T]?: T[P] extends Array<infer U> ?
        Array<StructProps<U>> : T[P] extends ReadonlyArray<infer U> ?
            ReadonlyArray<StructProps<U>> : StructProps<T[P]>;
};
...
export function createStruct<T>(
    ...
    for (const field of getStructFields(structType)) {
        ...
        try {
            ...
            if (error instanceof ValidationError) {
                throw new ValidationError(
                    () => `At ${field.propertyKey} (of ${field.target.name}): \n\t${error.validation()
                        .replace(/\n\t/g, '\n\t\t')}`
                )
            }
            throw error;
        ...
        if (value !== undefined)
            struct[field.propertyKey] = value;
    }
