...
export const getFieldDesignType = WeakMapFactory((field: Field):
  | Function
  | undefined => {
...
export const getFieldDesignOrForwardType = WeakMapFactory((field: Field):
  | Function
  | undefined => {
...
export function Forward<T>(
  ...
  target: Partial<Record<K, T | Record<string, T> | Map<any, T> | Iterable<T>>>,
  propertyKey: K
) => void {
  return (target, propertyKey) => {
    forwardTypes.set(getField(target.constructor, propertyKey), getType);
  };
}
