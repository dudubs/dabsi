...
testm(__filename, () => {
  ...
  it("EntityDataInfo.nonRelationColumnKeys", () => {
    ...
    const dChild1Child1Info = getEntityDataInfo(
  ...
  describe("relations", () => {
    ...
    // BABToOneRelations
    const EDEToOneRelations = mapObject(
      EDToOneRelations,
      () => DEToOneRelations
    );
  ...
  function testEntity<T>(
    ...
    beforeAll(async () => {
      ...
      const qb = DataQueryBuilder.createRoot(
        connection.getMetadata(typeInfo.type).tableName
      );
      const loader = EntityDataLoader.create({
        connection,
        typeInfo,
        qb,
        selection: <any>selection,
        schema: qb.query.alias,
        baseRow: {},
      });
      entityKeyToRow = mapArrayToObject(await loader.loadRows(), row => [
        row.$key,
        row,
      ]);
    });
