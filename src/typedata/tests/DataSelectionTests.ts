...
testm(__filename, () => {
  describe("childAt sanity:", () => {
    describe("picking:", () => {
      ...
      test(
        "pick and pick",
        { dPick: ["dText"], dChild1Pick: ["dText", "dText1Child"] },
        test => {
          test("xChild").toEqual(["dText"]);
      ...
      function test(
        title,
        options: { dPick?: string[]; dChild1Pick?: string[] },
        callback: (tester: (childKey: string) => jasmine.Matchers<any>) => void
      ) {
        it(title, () => {
          callback(childKey => {
            return expect(
              DataSelection.atChild(
                {
                  pick: options.dPick,
    ...
    it("relations", () => {
      ...
      expect([...dChild1.pick!].sort()).toEqual(["p_d", "p_dChild1"].sort());
