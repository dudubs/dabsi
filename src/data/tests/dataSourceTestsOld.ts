...
export abstract class DataSourceTesterOld {
...
export function DataSourceTestsOld(
    getTester: () => DataSourceTesterOld
) {
    ...
    it('sanity', async () => {
        ...
        function getMsgCountComments(msgId) {
            return tester.msgs.select({countComments: {$count: "comments"}}).get(msgId)
                .then(row => defined(row,'No row').countComments)
        }
    ...
    it('tryUndefined to update primary columns', async () => {
