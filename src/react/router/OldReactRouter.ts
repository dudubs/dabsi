...
export type ReactRouterRenderer<T extends AnyReactRouterOld> = (
  props: ReactRouterRendererProps<T>
) => ReactNode;

export type AnyReactRoute = Route<AnyReactRouterOld>;

export type OldReactRouter = Router &
  RouterWithRouterType<{
...
export type AnyReactRouterOld = AnyRouter & OldReactRouter;

export const ReactRouterRenderers = WeakMapFactory(
  (router: AnyRouter): ReactRouterRenderer<any>[] => []
);

export const OldReactRouter: OldReactRouter = Router.extend({
  render: reactRouterRender,
  renderIndex: createReactRouterRendererHook(
    (router) => router.isIndex,
    (router) => router.configure({ hasIndex: true })
  ),
  renderDefault: createReactRouterRendererHook((router) => router.isDefault),
...
export type ReactRouterRenderHook = <T extends AnyReactRouterOld>(
  this: T,
  callback: (props: ReactRouterRendererProps<T>) => ReactNode
) => T;
