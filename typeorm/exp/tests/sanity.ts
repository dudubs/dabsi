...
const MOVIES: Array<DeepPartial<Movie>> = [
    {
        ...
        stars: [person("Leonardo DiCaprio"), person("Kate Winslet")],
...
it('$isNot', async () => {
    expect(await QB(Movie)
        .whereExp({$isNot: firstMovie})
        .getOne()
    ).not.toEqual(firstMovie);
})
...
it('$count', async () => {
    expect(await QB(Movie)
        ...
        .addSelectExp({$count: "writers"},
            "countWriters").getRawMany()).toEqual(
