...
import {AnyInput, InputData, InputError, InputType, TInput} from "./Input";

export type InputViewProps<C extends RpcConnection<AnyInput>> =
    WidgetViewProps<C> & {


    onChange?(data: InputData<C>): void;

    inputRef?: (input: InputView<C> | null) => void;

    error?: InputError<C>;

    errorMap?: { [K in Extract<InputError<C>, string>]?: ReactNode }

    renderError?(error: InputError<C>): ReactNode;
};
