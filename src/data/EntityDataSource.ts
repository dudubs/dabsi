...
export class EntityDataSource<T> extends DataSource<T> {
    ...
    @Lazy() get entityInfo(): EntityDataSourceInfo<T> {
        ...
        for (let key of this.getCursor().path) {
    ...
    createRowLoader<Fields extends DataFields<T>>(qb: SelectQueryBuilder<T>,
                                                  fields: Fields):
    ...
    async query<Fields extends DataFields<T>>(query: DataQuery<T, Fields>):
        ...
        const take = query.keys?.length ?? query.take;
        if (take) {
            qb.take(take);
        }
        ...
        const {expressionMap: {selects}} = qb;
        selects.length = 0;
        const getValues = this.createRowLoader(qb, query.fields);
