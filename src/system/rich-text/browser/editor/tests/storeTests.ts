...
describe("query", () => {
  ...
  const t = Tester.beforeAll(() => {
    ...
    return {
      selectOneBlock: store
        .select({ anchorKey: "sk1" })
        .selectedBlocks.map(b => b.key),
      selectTwoBlocks: store
        .select({ anchorKey: "sk1", focusKey: "sk2" })
        .selectedBlocks.map(b => b.key),
    };
  });

  it("expect to select 1 blocks", () => {
    expect(t.selectOneBlock).toEqual(["sk1"]);
  });
  it("expect to select 2 blocks", () => {
    expect(t.selectTwoBlocks).toEqual(["sk1", "sk2"]);
  });
});
...
describe("insertBlocks", () => {
  ...
  const getBlockTexts = () =>
    store.rawContent.blocks.map(b => b.text).join("\n");
...
describe("change block type:", () => {
  const t = Tester.beforeAll(() => {
    ...
    const rawContent = store.rawContent;
