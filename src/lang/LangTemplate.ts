...
export type LangTemplate<K extends string> = {
  ...
  // provider
  (strings: TemplateStringsArray, ...keys: K[]): LangTemplateEntry<K>;
};

export type LangTemplateFormatter<K extends string> = (
  props: Record<K, any>
) => string;

export type LangTemplateEntry<K extends string> = [
  string,
  LangTemplateFormatter<K>
];
...
export type LangTemplateProps<K extends string> = {
  ...
  props: Record<
    K,
    LangTemplateElement<any> | LangTokenElement | string | number
  >;
  params: K[];
  strings: ReadonlyArray<string>;
};

export type LangTemplateElement<K extends string> = ReactElement<
  LangTemplateProps<K>
>;

export function LangTemplate<K extends string>(
  strings: ReadonlyArray<string>,
  params: K[]
): LangTemplate<K> {
