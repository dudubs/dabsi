import {If, IsEmptyObject, OmitKeys, PartialUndefinedKeys} from "../../common/typings";
...
type BaseTableConfig<T, D, DS extends DataSelection<D>> =
    DataTableConfig<T & BaseRow, NoRpc, D, DS>;

type BaseTable<T> = DataTable<T & BaseRow, NoRpc>;

export type DataInputConfig<T, D,
    DS extends DataSelection<D>,
    TableConfig extends BaseTableConfig<T, D, DS> = BaseTableConfig<T, D, DS>> =
    PartialUndefinedKeys<{
        columns: OmitKeys<TableConfig['columns'], "label"> |
...
export type DataInput<T, N extends boolean,
    Table extends BaseTable<T> = BaseTable<T>> = NullableInput<N, {
