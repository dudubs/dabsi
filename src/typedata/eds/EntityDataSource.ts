...
import { DataQueryBuilderOld } from "../../typeorm/DataQueryBuilder";
import { EntityRelation } from "../../typeorm/relations";
import { DataCursor, EmptyDataCursor } from "../DataCursor";
import { DataKey } from "../DataKey";
import { BaseDataRow, DataBaseRow, DataRow } from "../DataRow";
import { DataValues } from "../DataSource";
import { AbstractDataSource, GetDataSource } from "../DataSource/DataSource";
...
export class EntityDataSource<T> extends AbstractDataSource<T> {
  ...
  createDataQueryBuilder(): DataQueryBuilderOld {
    return EntityDataCursor.createDataQueryBuilder(this.entityCursor);
  }

  createEntityLoader() {
    return EntityDataSelector.selectCursor(
      this.entityCursor.typeInfo,
      this.createDataQueryBuilder(),
  ...
  getCount(): Promise<number> {
    return this.createDataQueryBuilder().getCount();
  }

  hasRows(): Promise<boolean> {
    return this.createDataQueryBuilder().hasRows();
  }
