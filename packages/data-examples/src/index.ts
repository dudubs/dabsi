...
(async () => {
    connection = await createConnection({
        ...
        synchronize: true,
        database: ":memory:",
        entities: [BaseAnimal, Dog, Cat,
            DogBreed,
            Dog, PurebredDog, MixedDog]
    });
    ...
    const PurebredDogsOfHusky = PurebredDogs
        .of("breed", dogBreedsKeys.husky);
    ...
    const huskeyDogs = await Animals
        .filter({
            $or: [
